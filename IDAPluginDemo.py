        ...
        if(start<=addr<=end):
            ...
            try:
                ...
                if print_insn_mnem(addr) == 'jmp' and op1 == op2:
                    ...
                    size = j_end-j_start
                    print("%X %s size=%X\t" % (addr, generate_disasm_line(addr,0),size))
                    if j_start<j_end:
                        #nop(j_start,j_end)
...
def test_pe_win_x8664_driver():
    ...
    # libcache=True,
    ql = Qiling(["./examples/rootfs/x8664_windows/bin/navagio.sys"], "./examples/rootfs/x8664_windows", stop=QL_STOP.STACK_POINTER, 
